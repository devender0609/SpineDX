...
if (-not (Test-Path "$source\lib\validation.ts")) { throw "v27.7 validation module is missing." }
if (-not (Test-Path "$source\components\SpineDecisionApp.tsx")) { throw "v27.7 application component is missing." }
if (-not (Test-Path "$destination\.git")) { throw "Destination is not the permanent Git repository." }

Set-Location $destination
if (git status --porcelain) { throw "The destination repository has uncommitted changes. Commit, stash, or discard them before installing v27.7." }
...
Write-Host "v27.7 copied and verified successfully." -ForegroundColor Green
Write-Host "Next: git add -A; git commit -m 'Fix v27.7 assessment layout and responsive navigation'; git push origin main"
