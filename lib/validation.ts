...
const rangeRules:RangeRule[] = [
  ...
  {field:"baselineOdi",label:"Baseline ODI",min:0,max:100,domain:"symptoms"},
  {field:"baselinePromisPf",label:"Baseline PROMIS Physical Function",min:0,max:100,warningMin:20,warningMax:80,domain:"symptoms"},
  {field:"baselinePromisPi",label:"Baseline PROMIS Pain Interference",min:0,max:100,warningMin:20,warningMax:80,domain:"symptoms"},
...
export function validateCaseInput(i:CaseInput):ValidationIssue[]{
  ...
  if(i.lumbarScopeConfirmed==="not-assessed") add(issues,{id:"scope-unconfirmed",severity:"error",domain:"orientation",field:"lumbarScopeConfirmed",title:"Lumbar scope is not confirmed",message:"Confirm whether this assessment is primarily lumbar/lumbosacral before the lumbar pathway is interpreted.",action:"Answer the lumbar scope confirmation."});
  if(i.lumbarScopeConfirmed==="no"||i.lumbarScopeConfirmed==="uncertain") add(issues,{id:"scope-outside",severity:"error",domain:"orientation",field:"lumbarScopeConfirmed",title:"This case is outside the current lumbar/lumbosacral module",message:"A region-specific assessment is required. Safety documentation may continue, but lumbar localization, decompression, fusion, and treatment synthesis are withheld.",action:"Complete safety documentation and refer to the appropriate region-specific pathway."});
