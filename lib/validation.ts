...
export function validateCaseInput(i:CaseInput):ValidationIssue[]{
  ...
  if(i.primaryRegion!=="not-assessed"&&i.primaryRegion!=="lumbar") add(issues,{id:"region-outside",severity:"warning",domain:"orientation",field:"primaryRegion",title:"Current presentation is not primarily lumbar",message:"Safety screening may continue, but lumbar localization and treatment synthesis will be limited.",action:"Confirm the region or use the appropriate pathway."});
  ...
  if(incomplete.length) add(issues,{id:"safety-incomplete",severity:"warning",domain:"safety",title:"Emergency assessment is incomplete",message:`${incomplete.length} required safety item(s) remain unknown or not assessed.`,action:"Complete the safety screen before interpreting urgency as routine."});
  ...
  if(i.injectionResponse!=="not-tried"&&i.injectionResponse!=="unknown"&&i.injectionLevel==="unknown") add(issues,{id:"injection-level-missing",severity:"warning",domain:"treatment",title:"Injection response lacks a documented target",message:"A response is recorded without the injection level.",action:"Enter the level and side or mark them unknown."});
