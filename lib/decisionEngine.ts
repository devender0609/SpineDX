...
export type CaseInput = {
  ...
  plannedProcedure:"not-selected"|"decompression"|"discectomy"|"fusion"|"decompression-fusion"|"other"; sexAtBirth:"unknown"|"female"|"male"|"intersex"; plannedLevels:number; plannedApproach:"not-selected"|"posterior"|"anterior"|"lateral"|"combined"; plannedRevision:boolean; plannedSetting:"outpatient"|"inpatient"|"not-selected";
  ...
  clinicianAgreement:"not-reviewed"|"agree"|"partly-agree"|"disagree"; overrideReasonCategory:"none"|"incorrect-syndrome"|"incorrect-level"|"incorrect-root"|"imaging-overcalled"|"imaging-undercalled"|"competing-diagnosis"|"deficit-severity"|"urgency"|"treatment-threshold"|"patient-preference"|"medical-risk"|"missing-information"|"other"; overrideReason:string; finalClinicalDecision:string;
};
...
export function validateCase(i:CaseInput){const e:string[]=[];if(i.age<18)e.push("Enter and confirm an adult patient age.");if(i.bmi<=0)e.push("Enter BMI or document it as unavailable in the research record.");if(!i.patientGoal.trim())e.push("Document the patient-defined goal.");if(!i.actualImagesReviewed)e.push("Confirm direct image review or document why it is unavailable.");if(!i.levelByLevelDocumented)e.push("Complete the level-by-level imaging matrix.");if(i.examConfidence==="low")e.push("Repeat or confirm the neurologic examination because confidence is low.");return e}
