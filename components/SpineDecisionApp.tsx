...
export default function SpineDecisionApp() {
  ...
  return (
    <main>
      ...
      <div className="layout">
        <form className="form" onSubmit={generate} noValidate>
          <Section title="Patient and symptoms" step={1}>
            <div className="grid">
              ...
              <Field label="Clinician-entered symptom pattern"><select value={data.painPattern} onChange={e => update("painPattern", e.target.value as CaseInput["painPattern"])}><option>L3</option><option>L4</option><option>L5</option><option>S1</option><option value="claudication">Neurogenic claudication</option><option value="axial">Predominantly axial</option></select></Field>
          ...
          <Section title="Prior care and safety screen" step={4}>
            ...
            <div className="red-flag-box"><strong>Urgent safety screen</strong>
              ...
              <label className="check"><input type="checkbox" checked={data.feverOrInfectionRisk} onChange={e => update("feverOrInfectionRisk", e.target.checked)} /> Fever, bacteremia, immunosuppression, recent procedure, or other infection risk</label>
            </div>
          </Section>
        ...
        <aside className="results" ref={resultsRef} aria-live="polite">
          ...
          {result && <>
            ...
            <div className="score-card"><div className="score-heading"><div><span>Clinical–imaging reconciliation</span><strong>{result.matchedDomains}</strong><small>/{result.assessableDomains} assessable domains matched</small></div><b className={`label ${result.concordanceLabel}`}>{result.concordanceLabel}</b></div><div className="meter"><i style={{ width: `${result.assessableDomains ? (result.matchedDomains / result.assessableDomains) * 100 : 0}%` }} /></div><small>Unvalidated reconciliation aid—not a probability, diagnosis, surgical indication, or authorization criterion.</small></div>
      ...
      <footer><strong>Logic version 0.2 — literature-aligned prototype.</strong> Rules remain unvalidated and require multidisciplinary clinical governance, prospective validation, cybersecurity review, privacy controls, and regulatory assessment before patient-care use.</footer>
    </main>
  );
}
