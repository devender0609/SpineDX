...
const initialCase: CaseInput = {
  ...
  rightHipFlexion: "5", leftHipFlexion: "5", rightKneeExtension: "5", leftKneeExtension: "5",
  rightAnkleDorsiflexion: "4", leftAnkleDorsiflexion: "5", rightGreatToeExtension: "4", leftGreatToeExtension: "5",
  rightPlantarFlexion: "5", leftPlantarFlexion: "5", weaknessQuality: "true", weaknessTrajectory: "stable",
  rightPatellarReflex: "normal", leftPatellarReflex: "normal", rightAchillesReflex: "normal", leftAchillesReflex: "normal",
  sensoryRoot: "L5", straightLegRaise: "positive", femoralStretch: "negative", gaitAbnormal: false, heelWalkAbnormal: true,
  toeWalkAbnormal: false, repeatedHeelRaiseAbnormal: false, hipExamAbnormal: false, pulsesAbnormal: false, neuropathyFeatures: false,
  standingProvocationPattern: "both", reliefPattern: "sitting", bicycleToleranceBetter: true,
  imagingAgeMonths: 2, actualImagesReviewed: false, imageQuality: "adequate", levelByLevelDocumented: false, imagingLevel: "L4-5", imagingSide: "right", imagingFinding: "lateral-recess", stenosisSeverity: "severe",
  migratedDisc: false, spondylolisthesis: true, slipMillimeters: 4, dynamicInstability: "unknown", translationMillimeters: 0,
  angularMotionDegrees: 0, deformityPresent: false, coronalCobbDegrees: 0, lateralListhesisMillimeters: 0, segmentalKyphosisDegrees: 0, sagittalImbalancePresent: false, foraminalCollapse: "unknown", priorLumbarSurgery: false, plannedFacetResection: "unknown",
  completedExerciseProgram: true, exerciseWeeks: 12, medicationTrial: true, injectionResponse: "meaningful-temporary",
  injectionLevel: "L4-5", injectionSide: "right", injectionDurationDays: 21, injectionType: "transforaminal", injectionImmediateReliefPercent: 80, injectionDelayedReliefPercent: 60, injectionFunctionImproved: true,
  progressiveWeakness: false, urinaryRetention: false, urinarySensationLoss: false, urinaryInitiationDifficulty: false, overflowIncontinence: false, urgencyAlone: false, saddleAnesthesia: false, bilateralSevereDeficit: false, fever: false,
...
export default function SpineDecisionApp() {
  ...
  const surrogate = useMemo(() => snapshot && result ? runConservativeSurrogate(snapshot, result) : null, [snapshot, result]);
    ...
    <div className={`workspace ${result ? "has-results" : "form-only"}`}>
      ...
      <form className="clinical-form" onSubmit={submit} noValidate>
        ...
        {stage === "exam" && <div className="stage-stack">
          <div className="stage-header"><div><span className="stage-kicker">Step 2</span><h2>Examination</h2><p>Record side-specific motor, reflex, sensory, gait, and competing-source findings.</p></div><Pill tone="neutral">Bilateral examination required</Pill></div>
          <Card title="Bilateral motor examination" eyebrow="MRC GRADES — DOCUMENT PAIN-LIMITED TESTING">
            <div className="grid three">
              {([
                ["rightHipFlexion","Right hip flexion"],["leftHipFlexion","Left hip flexion"],
                ["rightKneeExtension","Right knee extension"],["leftKneeExtension","Left knee extension"],
                ["rightAnkleDorsiflexion","Right ankle dorsiflexion"],["leftAnkleDorsiflexion","Left ankle dorsiflexion"],
                ["rightGreatToeExtension","Right great-toe extension"],["leftGreatToeExtension","Left great-toe extension"],
                ["rightPlantarFlexion","Right plantar flexion"],["leftPlantarFlexion","Left plantar flexion"]
              ] as const).map(([key,label])=><Field key={key} label={label}><select value={data[key]} onChange={e=>update(key,e.target.value as CaseInput[typeof key])}>{grades.map(x=><option key={x} value={x}>{x}</option>)}</select></Field>)}
              <Field label="Weakness quality"><select value={data.weaknessQuality} onChange={e=>update("weaknessQuality",e.target.value as CaseInput["weaknessQuality"])}><option value="true">Likely true weakness</option><option value="pain-limited">Pain limited</option><option value="uncertain">Uncertain</option></select></Field>
              <Field label="Trajectory"><select value={data.weaknessTrajectory} onChange={e=>update("weaknessTrajectory",e.target.value as CaseInput["weaknessTrajectory"])}><option value="none">No weakness</option><option value="stable">Stable</option><option value="progressive">Progressive</option><option value="improving">Improving</option></select></Field>
            </div>
          </Card>
          <Card title="Reflexes, sensation, and tension signs">
            <div className="grid three">
              {([ ["rightPatellarReflex","Right patellar"],["leftPatellarReflex","Left patellar"],["rightAchillesReflex","Right Achilles"],["leftAchillesReflex","Left Achilles"] ] as const).map(([key,label])=><Field key={key} label={label}><select value={data[key]} onChange={e=>update(key,e.target.value as CaseInput[typeof key])}>{reflexes.map(x=><option key={x} value={x}>{x}</option>)}</select></Field>)}
              <Field label="Sensory pattern"><select value={data.sensoryRoot} onChange={e=>update("sensoryRoot",e.target.value as CaseInput["sensoryRoot"])}>{["L1","L2","L3","L4","L5","S1","none","non-dermatomal","not-tested"].map(x=><option key={x} value={x}>{x}</option>)}</select></Field>
          ...
          <Card title="Functional examination and mimics">
            <div className="grid three"><Field label="Provocation"><select value={data.standingProvocationPattern} onChange={e=>update("standingProvocationPattern",e.target.value as CaseInput["standingProvocationPattern"])}><option value="not-assessed">Not assessed</option><option value="standing">Standing</option><option value="walking">Walking</option><option value="both">Standing and walking</option></select></Field><Field label="Relief pattern"><select value={data.reliefPattern} onChange={e=>update("reliefPattern",e.target.value as CaseInput["reliefPattern"])}><option value="not-assessed">Not assessed</option><option value="sitting">Sitting</option><option value="flexion">Flexion</option><option value="stopping-only">Stopping only</option><option value="none">No relief</option></select></Field></div>
            <div className="check-grid compact"><Check checked={data.gaitAbnormal} onChange={v=>update("gaitAbnormal",v)}>Abnormal gait</Check><Check checked={data.heelWalkAbnormal} onChange={v=>update("heelWalkAbnormal",v)}>Abnormal heel walk</Check><Check checked={data.toeWalkAbnormal} onChange={v=>update("toeWalkAbnormal",v)}>Abnormal toe walk</Check><Check checked={data.repeatedHeelRaiseAbnormal} onChange={v=>update("repeatedHeelRaiseAbnormal",v)}>Abnormal repeated heel raises</Check><Check checked={data.hipExamAbnormal} onChange={v=>update("hipExamAbnormal",v)}>Abnormal hip examination</Check><Check checked={data.pulsesAbnormal} onChange={v=>update("pulsesAbnormal",v)}>Abnormal pulses/vascular examination</Check><Check checked={data.neuropathyFeatures} onChange={v=>update("neuropathyFeatures",v)}>Peripheral neuropathy features</Check><Check checked={data.bicycleToleranceBetter} onChange={v=>update("bicycleToleranceBetter",v)}>Bicycle tolerance better than walking</Check></div>
          </Card>
        </div>}
        ...
        {stage === "imaging" && <div className="stage-stack">
          ...
          <Card title="Imaging governance" eyebrow="DIRECT REVIEW AND TARGET QUALITY">
            <div className="grid three"><Field label="Image quality"><select value={data.imageQuality} onChange={e=>update("imageQuality",e.target.value as CaseInput["imageQuality"])}><option value="adequate">Adequate</option><option value="limited">Limited</option><option value="unknown">Unknown</option></select></Field></div>
            <div className="check-grid compact"><Check checked={data.actualImagesReviewed} onChange={v=>update("actualImagesReviewed",v)}>Actual MRI/CT images reviewed by clinician</Check><Check checked={data.levelByLevelDocumented} onChange={v=>update("levelByLevelDocumented",v)}>Level-by-level side and zone matrix completed</Check></div>
          </Card>
          <Card title="Stability and operative anatomy">
            <div className="grid three"><Field label="Slip (mm)"><input type="number" min="0" value={data.slipMillimeters} onChange={e=>update("slipMillimeters",+e.target.value)}/></Field><Field label="Dynamic translation (mm)"><input type="number" min="0" value={data.translationMillimeters} onChange={e=>update("translationMillimeters",+e.target.value)}/></Field><Field label="Angular motion (°)"><input type="number" min="0" value={data.angularMotionDegrees} onChange={e=>update("angularMotionDegrees",+e.target.value)}/></Field><Field label="Foraminal collapse"><select value={data.foraminalCollapse} onChange={e=>update("foraminalCollapse",e.target.value as CaseInput["foraminalCollapse"])}><option value="unknown">Unknown</option><option value="absent">Absent</option><option value="present">Present</option></select></Field><Field label="Coronal Cobb (°)"><input type="number" min="0" value={data.coronalCobbDegrees} onChange={e=>update("coronalCobbDegrees",+e.target.value)}/></Field><Field label="Lateral listhesis (mm)"><input type="number" min="0" value={data.lateralListhesisMillimeters} onChange={e=>update("lateralListhesisMillimeters",+e.target.value)}/></Field><Field label="Segmental kyphosis (°)"><input type="number" value={data.segmentalKyphosisDegrees} onChange={e=>update("segmentalKyphosisDegrees",+e.target.value)}/></Field><Field label="Planned facet resection"><select value={data.plannedFacetResection} onChange={e=>update("plannedFacetResection",e.target.value as CaseInput["plannedFacetResection"])}><option value="unknown">Unknown</option><option value="limited">Limited</option><option value="substantial">Substantial</option></select></Field></div>
            <div className="check-grid compact"><Check checked={data.migratedDisc} onChange={v=>update("migratedDisc",v)}>Migrated or sequestered disc</Check><Check checked={data.spondylolisthesis} onChange={v=>update("spondylolisthesis",v)}>Degenerative spondylolisthesis</Check><Check checked={data.sagittalImbalancePresent} onChange={v=>update("sagittalImbalancePresent",v)}>Sagittal imbalance documented</Check><Check checked={data.deformityPresent} onChange={v=>update("deformityPresent",v)}>Clinically relevant deformity</Check><Check checked={data.priorLumbarSurgery} onChange={v=>update("priorLumbarSurgery",v)}>Prior lumbar surgery</Check></div>
          </Card>
        </div>}
      ...
      {result && <section className="results" ref={resultsRef} aria-live="polite">
        <>
          ...
          <div className="result-tabs"><button className={resultTab==="assessment"?"active":""} onClick={()=>setResultTab("assessment")}>Assessment</button><button className={resultTab==="management"?"active":""} onClick={()=>setResultTab("management")}>Management</button><button className={resultTab==="hybrid"?"active":""} onClick={()=>setResultTab("hybrid")}>Simulation</button><button className={resultTab==="evidence"?"active":""} onClick={()=>setResultTab("evidence")}>Evidence</button></div>
          ...
          {resultTab === "hybrid" && synthetic && surrogate && <div className="result-stack">
            <Card title="Conservative trained surrogate" eyebrow="SYNTHETIC-LABEL MODEL — NOT CLINICAL AI" tone="warning">
              <p className="lead">{surrogate.notice}</p>
              <div className="model-meta"><span><b>{surrogate.trainingRecords.toLocaleString()}</b> synthetic training records</span><span><b>{surrogate.holdoutRecords.toLocaleString()}</b> synthetic holdout records</span><span><b>{surrogate.modelVersion}</b></span></div>
            </Card>
            <Card title="Conservative surrogate pathway estimates" eyebrow="TREE OUTPUT — NOT TREATMENT BENEFIT">
              <div className="pathway-bars">{surrogate.pathwayEstimates.map(x=><div className="pathway-row" key={x.key}><div className="pathway-label"><b>{x.label}</b><span>{x.score}%</span></div><div className="pathway-track"><i style={{width:`${x.score}%`}} /></div><small>{x.rationale} Synthetic test AUROC {x.performance.aurocMean.toFixed(3)} (range {x.performance.aurocRange[0].toFixed(3)}–{x.performance.aurocRange[1].toFixed(3)}); sensitivity {x.performance.sensitivity.toFixed(3)}, specificity {x.performance.specificity.toFixed(3)}.</small></div>)}</div>
            </Card>
            <div className="two-col"><Card title="Input-mapping limitations"><List items={surrogate.mappingWarnings}/></Card><Card title="Mapped features used"><List items={surrogate.mappedInputs.slice(0,14)} empty="No active mapped feature was identified."/></Card></div>
            <Card title="Rule-sensitivity simulation" eyebrow="RESEARCH AND SAFETY-TEST MODULE" tone="soft">
              <p className="lead">{synthetic.modelNotice}</p>
              <div className="model-meta"><span><b>{synthetic.cohortSize.toLocaleString()}</b> simulated patient-level analogs</span><span><b>{synthetic.inputCompleteness}</b> input completeness</span><span><b>{synthetic.modelVersion}</b> model version</span></div>
            </Card>
            <Card title="Rule sensitivity across simulated scenarios" eyebrow="PROGRAMMED RULE SATISFACTION — NOT CLINICAL PROBABILITY">
            ...
            <div className="two-col"><Card title="Index phenotype"><p>{synthetic.indexPhenotype}</p></Card><Card title="Uncertainty drivers"><List items={synthetic.uncertaintyDrivers} empty="No major uncertainty driver was triggered by the current fields."/></Card></div>
            <Card title="Synthetic cohort composition" eyebrow="AGGREGATE PHENOTYPE CHECK"><div className="pathway-bars">{synthetic.cohortMetrics.map(x=><div className="pathway-row" key={x.label}><div className="pathway-label"><b>{x.label}</b><span>{x.value}%</span></div><div className="pathway-track"><i style={{width:`${x.value}%`}} /></div><small>{x.note}</small></div>)}</div></Card>
            <details className="detail-block"><summary>Simulation audit checks</summary><div className="detail-body"><List items={synthetic.auditChecks}/></div></details>
            <Card title="Outcome prediction status" tone="warning"><p>{synthetic.outcomeStatus}</p></Card>
            <details className="detail-block"><summary>How this module works</summary><div className="detail-body"><ul><li>Patient inputs are passed first through the safety and literature-rule engine.</li><li>A deterministic Monte Carlo stress test varies prespecified inputs around the index phenotype. It is not a cohort of observed patients and is not a nearest-neighbor model.</li><li>The displayed percentages summarize simulated scenario agreement, not treatment benefit, complication risk, or causal effect.</li><li>Safety rules override the synthetic model. No autonomous order or procedure recommendation is generated.</li></ul></div></details>
          </div>}
