...
if (-not (Test-Path "$source\lib\validation.ts")) { throw "v27.5 validation module is missing." }
if (-not (Test-Path "$source\components\SpineDecisionApp.tsx")) { throw "v27.5 application component is missing." }
if (-not (Test-Path "$destination\.git")) { throw "Destination is not the permanent Git repository." }

Set-Location $destination
if (git status --porcelain) { throw "The destination repository has uncommitted changes. Commit, stash, or discard them before installing v27.5." }
...
Write-Host "v27.5 copied and verified successfully." -ForegroundColor Green
Write-Host "Next: git add -A; git commit -m 'Harden v27.5 safety, region scope, and clinical logic'; git push origin main"
